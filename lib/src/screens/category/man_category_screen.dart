...
class ManCategoryScreen extends StatelessWidget {
  const ManCategoryScreen({Key? key}) : super(key: key);
        ...
        child: Consumer<List<Product>?>(builder: (context, products, _) {
          if (products != null) {
            if (products.isNotEmpty) {
              List<Product> manCategory = products.where((product) {
                return product.category.toLowerCase() == 'man';
              }).toList();
              if (manCategory.isEmpty) return const NoDataWidget();
              return GridView.builder(
                ...
                itemCount: manCategory.length,
                physics: const BouncingScrollPhysics(),
                itemBuilder: (context, index) {
                  Product product = manCategory[index];
                  return ProductItem(
                    title: product.title,
                    isGrid: true,
