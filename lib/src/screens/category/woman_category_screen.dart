...
class WomanCategoryScreen extends StatelessWidget {
  const WomanCategoryScreen({Key? key}) : super(key: key);
        ...
        child: Consumer<List<Product>?>(builder: (context, products, _) {
          if (products != null) {
            if (products.isNotEmpty) {
              List<Product> womanCategory = products.where((product) {
                return product.category.toLowerCase() == 'woman';
              }).toList();
              if (womanCategory.isEmpty) return const NoDataWidget();
              return GridView.builder(
                ...
                itemCount: womanCategory.length,
                physics: const BouncingScrollPhysics(),
                itemBuilder: (context, index) {
                  Product product = womanCategory[index];
                  return ProductItem(
                    title: product.title,
                    isGrid: true,
